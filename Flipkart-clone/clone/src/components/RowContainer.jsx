...
const RowContainer = ({ flag, scrollValue1 }) => {
  ...
  const filteredData = flag
    ? data.filter((item) => item.category === flag)
    : data;
    ...
    >
      <div className="flex items-center justify-evenly cursor-pointer space-x-6 px-4">
        {filteredData.map((item, index) => (
          <div
            key={index}
            className="w-[250px] h-[340px] bg-white rounded-lg p-2 px-4 border-solid border-2 border-gray-400"
          >
